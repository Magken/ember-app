...
export const CheckButton: React.FC<CheckButtonProps> = ({
  ...
  // Generate flame licks along checkbox edges
    ...
    >
      ...
      >
        ...
        {[...Array(emberCount)].map((_, i) => {
          ...
          const duration = (burst ? 0.8 : 2) + Math.random() * 0.8;

          return (
            <span
              key={i}
              className={`absolute w-[2px] h-[2px] ${color} rounded-full pointer-events-none animate-emberFromEdge mix-blend-screen`}
              style={{
                left: `${x}%`,
                top: `${y}%`,
