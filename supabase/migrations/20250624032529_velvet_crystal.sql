/*
  ...
  1. New Tables
    - `user_profiles` - Extended user information with unique codes and nicknames
    - `user_agreements` - Terms & Conditions acceptance tracking with timestamps
    - `auth_attempts` - Authentication attempt logging for rate limiting and security monitoring
  ...
  2. Security Features
    ...
    - Authentication attempt logging with IP and user agent tracking

  3. Functions & Triggers
    - Automatic user profile creation on signup
    - Timestamp management for profile updates
    - Cleanup functions for expired tokens and sessions
    - Security validation functions

  4. Performance Optimizations
    - Strategic indexes for efficient queries
    - Optimized rate limiting queries
    - Session and token cleanup procedures
*/
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_user_profile_timestamp_trigger') THEN
    DROP TRIGGER update_user_profile_timestamp_trigger ON user_profiles;
  END IF;
END $$;
